...
test_plot <- ggarrange(per_dormant_cell_Stain_clean, per_dormant_16s_clean, per_dormant_culture_clean,
          ...
          ncol = 2, nrow = 2,
          legend = "none",
          ylab = "% Dormant Bacteria",
          common.legend = T,
          font.label = list(size = 14, face = "bold", color ="black"))

test_plot <- annotate_figure(test_plot, bottom = text_grob("Species", rot = 0, vjust = 1))
test_plot <- annotate_figure(test_plot, left = text_grob("% Dormant Bacteria", rot = 090, vjust = 1))
  
overall_title <- ggdraw() +
  draw_label("Percent Dormant Bacteria From Different Computation Methods", size = 12, fontface = "bold")
